...
<head>
    <meta charset="utf-8" />
    <title>Cek Badan Ideal Quintessence</title>
...
<body>
    ...
    <form action="" method="post">
        <div class="m-lg-5">
            ...
            <div class="mb-3">
                <label for="T-badan" class="form-label">Tinggi Badan Dalam Cm</label>
                <input class="form-control" type="text" id="T-badan" placeholder="Contoh: 180" name="t-badan" value="<?= $tinggi?>" required/>
            </div>
            <div class="mb-1">
                <label for="B-badan" class="form-label">Berat Badan Dalam Kg</label>
                <input class="form-control form-control-sm" id="B-badan" type="text" name="b-badan" placeholder="Contoh: 65" value="<?= $berat ?>" required/>
            </div>
    ...
    if (isset($_POST['submit'])) {
        $hasilUpdate = updateDataBMI($_POST, $id);
        $status = $hasilUpdate[1];
        $nama_status = readStatusBMI($status);
        $statusBMI = mysqli_fetch_array($nama_status);

        if ($hasilUpdate[0] >= 0) {
            // jika penambahan sukses, tampilkan alert
            $status = $statusBMI['status_bmi'];
            echo "
            <script>
                alert('Hasil BMI anda " . intval($hasilUpdate[2]) . ". Anda berada dalam kategori " . $status . "');
